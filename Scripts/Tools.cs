...
public partial class Tools : Control
{
            ...
            switch(Globals.Instance.CurrentState)
            {
                case Enums.ToolState.AddingNode:
                    if(Globals.Instance.IsObstructed)
                        break;
                    node = Globals.Instance.Node.Instantiate();
                    if(node == null)
                        break;
                ...
                case Enums.ToolState.AddingGenerator:
                    if(Globals.Instance.IsObstructed)
                        break;
                    node = Globals.Instance.Generator.Instantiate();
                    if(node == null)
                        break;
                    GeneratorNode generator = node as GeneratorNode;
                    generator.ID = GetID();
                    Globals.Instance.SetNextID(generator.ID);
                    generator.Text = "Generator";

                    size = generator.Size;
                    generator.Position = GetGlobalMousePosition() - (size/2);
                    if(Globals.Instance.ClickedId < 0)
                    {
                        Control c = generator.GetChild(4) as Control;
                        c.Visible = true;
                        Globals.Instance.LastGenerator = generator;
                    }
    ...
    void MoveNode(float alpha)
    {
        ...
        NodeProp c = Globals.Instance.GetNode(id) as NodeProp;
        ...
        {
            LineProp lineProp = Globals.Instance.GetEdge(item) as LineProp;
            lineProp.SetPointPosition(0, c.Position + (c.Size/2));
            if(lineProp.Editable)
            {
